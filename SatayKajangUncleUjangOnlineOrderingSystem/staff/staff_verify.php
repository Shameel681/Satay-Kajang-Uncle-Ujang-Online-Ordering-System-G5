...
if (isset($_GET['token']) && $_GET['token'] !== '') {
    ...
    $sql = "SELECT staff_id, is_verified FROM staff WHERE verify_token = ? LIMIT 1";
    ...
    if ($row = $result->fetch_assoc()) {
        if ((int)$row['is_verified'] === 1) {
            ...
            $update_sql = "UPDATE staff 
                           ...
                           WHERE staff_id = ? AND verify_token = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("is", $row['staff_id'], $token);
            $update_stmt->execute();

            if ($update_stmt->affected_rows > 0) {
                $message = "✅ Your staff email has been verified! You can now login.";
                $message_type = "success";
            } else {
    ...
    <main>
        <div class="verify-container">
            ...
            <?php if ($message_type === "success"): ?>
                <a href="staff_login.php" class="btn">Go to Login</a>
            <?php else: ?>
                <a href="staff_register.php" class="btn">Back to Register</a>
            <?php endif; ?>
        </div>
    </main>
</body>

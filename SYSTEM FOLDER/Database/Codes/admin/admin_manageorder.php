...
session_start(); // Tambah session_start() jika belum ada di connect.php
...
$admin_name = htmlspecialchars($_SESSION['admin_name'] ?? 'Admin'); // Default value jika admin_name tiada
...
if (isset($_POST['update_order_status'])) {
    ...
    if (in_array($new_status, $valid_statuses)) {
        // Sediakan query dan semak jika gagal (PENAMBAHAN SEMAKAN DARI STAFF CODE)
        ...
        if (!$stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error = "Failed to update order status: " . $stmt->error; // Tunjukkan ralat sebenar
            }
            $stmt->close();
        }
    } else {
                        ...
                        <div class="card-body">
                            ...
                            <?php if ($result->num_rows > 0): ?>
                                <?php while ($order = $result->fetch_assoc()): ?>
                                    ...
                                    <?php if ($items_stmt) $items_stmt->close(); // Tutup statement ?>
                                <?php endwhile; ?>
                            <?php else: ?>

...
// staff/staff_viewsale.php
session_start();
require_once '../connect.php';

// 1. Authentication Check (Adapted for staff)
$is_loggedin = isset($_SESSION['staff_loggedin']) && $_SESSION['staff_loggedin'] === true;
$staff_name = $is_loggedin ? htmlspecialchars($_SESSION['staff_name']) : ''; // Use staff_name
if (!$is_loggedin) {
    // Redirect staff to their login page or profile page when not logged in
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    foreach ($sales_data as $row) {
        $row_output = [
            $row['order_id'],
            $row['customer_id'],
            $row['customer_name'] ?? 'Guest (ID: ' . $row['customer_id'] . ')', // Handle potential NULL customer_name from LEFT JOIN
            number_format($row['total_amount'], 2), // Format amount
            $row['order_status'],
            $row['payment_status'],
            $row['order_date']
        ];
